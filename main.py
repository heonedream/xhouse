...
def crawl(cities, lianjia_types, lianjia_regions, pages_num, path, save_file):
    ...
    with open(path_file, "a+") as csvfile:
        ...
        writer.writerow(
            ["city", "zone", "data_el", "house_name", "data_link", "room_num", "built_year", "direction",
             "total_price",
    ...
    for link in url_lists:
        ...
        assert len(city)==len(data_el)==len(zone)==len(house_name)==len(data_link)==len(room_num)==len(built_year)==\
        ...
        for i in range(0, len(city)):
            data_list.append((city[i], zone[i], data_el[i], house_name[i], data_link[i], room_num[i], built_year[i], direction[i],
...
if __name__ =="__main__":
    ...
    lianjia_regions = ["longgangqu"]
    pages_num = 100
    path = "/Users/hewenjun/Documents/project/house-price"
    save_file = "ershoufang_futian.csv"

    crawl(cities, lianjia_types, lianjia_regions, pages_num, path, save_file)
